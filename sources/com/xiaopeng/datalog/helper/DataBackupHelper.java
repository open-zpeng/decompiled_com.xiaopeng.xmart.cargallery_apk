...
public class DataBackupHelper {
    ...
    public void initDir(String tag, Context context) {
        File datafileDir = new File(DATA_FILE_DIR + tag + MqttTopic.TOPIC_LEVEL_SEPARATOR + context.getPackageName());
    ...
    public void deleteExpiredFile(final String tag, Context context) {
        File canFileDir = new File(DATA_FILE_DIR + tag + MqttTopic.TOPIC_LEVEL_SEPARATOR + context.getPackageName());
        ...
        for (String fileName : fileNames) {
            if (isExpiredFile(fileName)) {
                ...
                boolean isDeleted = new File(DATA_FILE_DIR + tag + MqttTopic.TOPIC_LEVEL_SEPARATOR + context.getPackageName() + MqttTopic.TOPIC_LEVEL_SEPARATOR + fileName).delete();
                LogUtils.d(TAG, "file: " + fileName + " is expired, deleted " + isDeleted);
            }
        }
    ...
    /* JADX INFO: Access modifiers changed from: private */
    /* JADX WARN: Unsupported multi-entry loop pattern (BACK_EDGE: B:9:0x0069 -> B:26:0x007a). Please submit an issue!!! */
    ...
    private File createNewFile(String tag, Context context) {
        String dateStr = DateUtils.formatDate10(System.currentTimeMillis());
        return new File(DATA_FILE_DIR + tag + MqttTopic.TOPIC_LEVEL_SEPARATOR + context.getPackageName() + MqttTopic.TOPIC_LEVEL_SEPARATOR + tag + "_" + dateStr + ".log");
    }
