...
public final class XIndicatorDrawable extends Drawable {
    ...
    protected void onBoundsChange(Rect bounds) {
        ...
        if (this.mIndicatorType == 1) {
            ...
            this.mIndicatorRect.bottom = bounds.height();
        }
    ...
    public void setSelection(int indicatorCount, int selection, boolean animation) {
        ...
        if (this.mIndicatorCount > this.mCurrentSelection && rect.width() > 0) {
            ...
            float f = this.mCurrentSelection * segmentLength;
            this.mAnimIndicatorStart = f;
            float f2 = f + segmentLength;
            this.mAnimIndicatorEnd = f2;
            if (this.mIndicatorType == 1) {
                float padding = ((1.0f - this.mIndicatorPercent) * segmentLength) / 2.0f;
                this.mAnimIndicatorStart = f + padding;
                this.mAnimIndicatorEnd = f2 - padding;
            }
            float f3 = this.mIndicatorStart;
            float f4 = this.mAnimIndicatorStart;
            if (f3 == f4 && this.mIndicatorEnd == this.mAnimIndicatorEnd) {
                return;
            }
            if (animation) {
                startAnimation();
