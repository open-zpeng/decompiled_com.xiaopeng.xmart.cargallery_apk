...
public class SoundEffectPool extends AbsSoundEffect {
    ...
    private class LoaderAssets implements XSoundEffect.SoundLoader {
        ...
        public int load(SoundEffectResource resource) {
            if (SoundEffectPool.this.mSoundPool == null) {
                SoundEffectPool.this.log("LoaderAssets--resource:" + resource + ",mSoundPool is null");
                return 0;
            }
            AssetFileDescriptor fd = null;
            try {
                fd = Xui.getContext().getAssets().openFd(SoundEffectPool.this.mSoundEffectResource.getPath());
                SoundEffectPool.this.log("LoaderAssets--resource:" + resource + ",path:" + SoundEffectPool.this.mSoundEffectResource.getPath() + ", is success");
            } catch (IOException e) {
                SoundEffectPool.this.log("LoaderAssets--resource:" + resource + ",path:" + SoundEffectPool.this.mSoundEffectResource.getPath() + ", is error");
                e.printStackTrace();
            }
            try {
    ...
    private class LoaderSystem implements XSoundEffect.SoundLoader {
        ...
        public int load(SoundEffectResource resource) {
            ...
            SoundEffectPool.this.log("LoaderSystem--resource:" + resource + ",mSoundPool is null");
            return 0;
        }
    }
}
