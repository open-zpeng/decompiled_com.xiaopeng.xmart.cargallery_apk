...
public class XSegmented extends XLinearLayout implements IVuiElementListener, View.OnClickListener {
    ...
    public XSegmented(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public XSegmented(Context context, AttributeSet attrs, int defStyleAttr) {
        this(context, attrs, defStyleAttr, 0);
    }

    public XSegmented(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);
        XIndicatorDrawable xIndicatorDrawable = new XIndicatorDrawable();
        this.mIndicatorDrawable = xIndicatorDrawable;
        xIndicatorDrawable.inflateAttrs(getResources(), attrs, context.getTheme());
    ...
    public void addSegmentView(int[] icons, CharSequence[] vuiLabels) {
        this.mIconRes = icons;
        int i = 0;
    ...
    public void addSegmentView(int icon, CharSequence vuiLabel) {
        addSegmentView(icon, vuiLabel, getChildCount());
    }

    public void addSegmentView(int icon, CharSequence vuiLabel, int index) {
        ...
        if (index >= this.mIconRes.length) {
            ...
            int[] iArr = new int[iconRes.length + 1];
            this.mIconRes = iArr;
            System.arraycopy(iconRes, 0, iArr, 0, iconRes.length);
            this.mIconRes[iconRes.length] = icon;
        } else if (index < getChildCount()) {
            int[] iconRes2 = this.mIconRes;
            int[] iArr2 = new int[iconRes2.length + 1];
            this.mIconRes = iArr2;
            System.arraycopy(iconRes2, 0, iArr2, 0, index);
            int[] iArr3 = this.mIconRes;
            iArr3[index] = icon;
            System.arraycopy(iconRes2, index, iArr3, index + 1, iconRes2.length - index);
        }
    ...
    public void setSegmentIcon(int res, CharSequence vuiLabel, int index) {
        if (getChildCount() > index) {
            View view = getChildAt(index);
    ...
    public void removeSegment(int index) {
        if (getChildCount() > index) {
            ...
            if ((view instanceof ImageView) && this.mIconRes != null) {
                ...
                int[] iArr = new int[iconRes.length - 1];
                this.mIconRes = iArr;
                System.arraycopy(iconRes, 0, iArr, 0, index);
                int[] iArr2 = this.mIconRes;
                System.arraycopy(iconRes, index + 1, iArr2, index, iArr2.length - index);
            }
            removeViewAt(index);
            int selection = this.mSelection;
            if (index == selection) {
    ...
    private void setSelection(int index, boolean playAnimation, boolean fromUser, boolean isEdt) {
        ...
        while (i2 < getChildCount()) {
            getChildAt(i2).setSelected(this.mSelection == i2);
            i2++;
        }
    ...
    public VuiElement onBuildVuiElement(String vuiElementId, IVuiElementBuilder builder) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (childView instanceof VuiView) {
                ...
                XLogUtils.d(TAG, "onBuildVuiElement:" + vuiElementId);
            }
        }
        return null;
    }
