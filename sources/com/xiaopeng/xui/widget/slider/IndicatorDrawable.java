...
class IndicatorDrawable extends Drawable implements Drawable.Callback {
    ...
    private void resetBounds() {
        ...
        if (offsetStart < 0.0f) {
            this.mBounds.left = 0;
            this.mBounds.right = specifyWidth;
        } else if (offsetEnd < 0.0f) {
            this.mBounds.left = i - specifyWidth;
            this.mBounds.right = this.slideWidth;
        } else {
            this.mBounds.left = (int) (f - (specifyWidth / 2));
            this.mBounds.right = (int) (this.indicatorCenter + (specifyWidth / 2));
        }
        setBounds(this.mBounds);
    }
