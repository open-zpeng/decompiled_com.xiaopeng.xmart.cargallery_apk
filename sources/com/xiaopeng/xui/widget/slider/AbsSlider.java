...
public abstract class AbsSlider extends XViewGroup implements IVuiElementListener {
    ...
    private void setStyle(AttributeSet attrs, int styleRes) {
        ...
        if (!this.hidePop) {
            ...
            indicatorDrawable.inflateAttr(getResources(), getContext().getTheme(), attrs, styleRes);
    ...
    private void readStyleAttrs(AttributeSet attrs, int styleRes) {
        ...
        if (attributes.hasValueOrEmpty(R.styleable.XSlider_slider_thumb)) {
            int resourceId = attributes.getResourceId(R.styleable.XSlider_slider_thumb, 0);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        setPadding(0, 0, 0, 0);
        int i = (this.mOrientation == 0 ? w : h) - 32;
        this.workableTotalLength = i;
        this.bgItemGap = i / (this.itemCount - 1);
        int i2 = this.initIndex;
        int i3 = this.startIndex;
        this.mProgressViewLength = (Math.abs((i2 - i3) / (this.endIndex - i3)) * this.workableTotalLength) + 16.0f;
        for (int i4 = 0; i4 < this.itemCount; i4++) {
            SlideLineView slideLineView = new SlideLineView(getContext(), this.mProgressViewLength > (this.bgItemGap * ((float) i4)) + 16.0f, this.mTickMarkStyleRes);
    ...
    public void stickIndicator() {
        ...
        float natureGap = this.workableTotalLength / (this.endIndex - this.startIndex);
        float number = (this.mProgressViewLength - 16.0f) / natureGap;
        this.mProgressViewLength = (((int) number) * natureGap) + 16.0f;
    }
    ...
    private int resetAlpha(int color, int alpha) {
