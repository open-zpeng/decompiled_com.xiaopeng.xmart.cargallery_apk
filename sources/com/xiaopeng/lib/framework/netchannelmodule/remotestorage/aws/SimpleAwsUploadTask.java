...
public class SimpleAwsUploadTask extends BaseAwsTask {
    ...
    private JsonObject wrapJsonBody(UploadInfo uploadInfo) {
        ...
        jsonObject.addProperty(LoginManager.AUTH_INFO_EXTRA_APP_ID, uploadInfo.app_id);
        jsonObject.addProperty("file", uploadInfo.file);
        jsonObject.addProperty("model", uploadInfo.model);
        jsonObject.addProperty(BIConfig.PROPERTY.DATA_TYPE, uploadInfo.type);
    ...
    private String createSign(String file, String timestamp) {
        ...
        sb.append("file").append(file).append(BIConfig.PROPERTY.DATA_TYPE).append(UploadInfo.UPLOAD_TYPE).append("v").append("2");
        String appString = sb.toString();
