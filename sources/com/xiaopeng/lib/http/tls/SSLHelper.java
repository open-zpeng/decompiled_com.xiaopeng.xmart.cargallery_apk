...
public class SSLHelper {
    ...
    public static class TLS2SocketFactory extends SSLSocketFactory {
        ...
        public Socket createSocket(Socket s, String host, int port, boolean autoClose) throws IOException {
            return patch(this.delegate.createSocket(s, host, port, autoClose), host + ":" + port);
        }

        @Override // javax.net.SocketFactory
        public Socket createSocket(String host, int port) throws IOException {
            return patch(this.delegate.createSocket(host, port), host + ":" + port);
        }

        @Override // javax.net.SocketFactory
        public Socket createSocket(String host, int port, InetAddress localHost, int localPort) throws IOException {
            return patch(this.delegate.createSocket(host, port, localHost, localPort), host + ":" + port);
        }

        @Override // javax.net.SocketFactory
        public Socket createSocket(InetAddress host, int port) throws IOException {
            String h = (host == null ? new StringBuilder() : new StringBuilder().append(host.getHostName())).append(":").append(port).toString();
            return patch(this.delegate.createSocket(host, port), h);
        }

        @Override // javax.net.SocketFactory
        public Socket createSocket(InetAddress address, int port, InetAddress localAddress, int localPort) throws IOException {
            String h = (address == null ? new StringBuilder() : new StringBuilder().append(address.getHostName())).append(":").append(port).toString();
